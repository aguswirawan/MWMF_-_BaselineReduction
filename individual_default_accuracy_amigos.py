...
for label in ['all']: # ,'arousal','dominance']:#['valence','arousal','dominance']:
    ...
    for sub in subjects:
        print(sub)
        xl = xlrd.open_workbook(r'result_default_sub/Individual/sub_dependent_v0/yes/Data_Preprocessed_P'+sub+'_'+label+epoch+'/summary_Data_Preprocessed_P'+sub+'.xlsx')
        table = xl.sheets()[1]
...
writer = pd.ExcelWriter('individual_result_default_sub_'+label+'.xlsx')
